...
def main() -> int:
    ...
    ap.add_argument("--sheet", default="Диски")
    ...
    # fixed mapping for Centrshin "Диски"
    COL_NAME = 1
    COL_SKU = 2
    COL_BRAND = 3
    COL_QTY = 14
    ...
    stats: Dict[str, Any] = {
        ...
        "parser_id": "centrshin_diski_xlsx_v1",
        "parser_version": "1.0",
        "run_id": RUN_ID,
        "file": FILE,
        "sheet": SHEET,
    ...
    try:
        ...
        for r in range(2, max_row + 1):
            ...
            price_cell = ws.cell(row=r, column=COL_PRICE).value
            price_raw = norm(price_cell)

            # Treat explicit zero as its own bucket (do NOT count as missing)
            if price_raw in ("0", "0.0", "0,0"):
                stats["skipped_price_zero"] += 1
                continue

            price_s, pf = parse_price(price_cell)

            if "missing_price" in pf or price_s == "":
                stats["skipped_price_missing"] += 1
                continue

            # parse_price marks <=0 as bad_price, but we want explicit accounting
            if "bad_price" in pf:
                raw = norm(price_cell)
                if raw in ("0","0.0","0,0"):
                    stats["skipped_price_zero"] += 1
                else:
                    stats["bad_price_non_numeric"] += 1
                continue
