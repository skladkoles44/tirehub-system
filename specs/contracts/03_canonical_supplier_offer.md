...
## Semantic core
Canonical Supplier Offer описывает наблюдаемое supplier offer состояние по конкретному предложению и складу.
Минимальная смысловая ось этой сущности: кто предлагает, что предлагает, где доступно, в каком состоянии доступности находится предложение и как оно трассируется к intake object.
...
- `availability_status` должен отражать нормализованное состояние доступности предложения.
...
- Если одновременно отсутствуют `price_purchase_cents`, `stock_qty_normalized` и `stock_qty_raw`, запись не считается полноценным current offer без отдельной policy.
- Наличие trace-полей само по себе не делает запись пригодной для supplier current update.
...
- Гранулярность записи supplier layer соответствует одному `supplier_id + offer_key + warehouse_key`.
